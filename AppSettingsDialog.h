...
private:
    ...
    explicit AppSettingsDialog(QWidget *parent = nullptr) : QDialog(parent)
    {
        ...
        addSettingGroup(boxLayout, "autoScanLANDevices", "自动扫描局域网设备", {"关闭", "开启"}, 1);
        ...
        addSortableGroup(mainLayout, "sideBarMenu", "左侧栏 (拖拽调整)", 
            {"🔗设备连接", "🕹️同屏操作", "⚙️设置", "💡帮助", "📲越狱助手", "⏳续费", "💬客服", "📜日志", "🛠️开发者"}, false);
        ...
        addSortableGroup(mainLayout, "tabBarMenu", "分组标签页右键菜单 (拖拽调整)", 
            {"重命名分组", "添加分组", "删除分组", "投屏显示", "视频清晰度", "连接方式", "自动扫描局域网设备", "自动连接USB设备"}, true);
    }
